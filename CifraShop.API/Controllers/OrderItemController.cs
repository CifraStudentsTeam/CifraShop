...
namespace CifraShop.API.Controllers
{
    ...
    public class OrderItemController : ControllerBase
    {
        ...
        //ѕолучение состовл€€ющей заказа по id
        [HttpGet("by-id")]
        public async Task<ActionResult<OrderItem>> GetOrderItemById([FromQuery] int id)
        {
            ...
            if (orderItem == null) 
                return NotFound($"—остовл€юще€ заказа с id {id} не найден");

            return Ok(orderItem);
        }

        //ѕолучение состовл€юющей заказа по id заказа
        [HttpGet("by-orderid")]
        public async Task<ActionResult<List<OrderItem>>> GetOrderItemsByOrderId([FromQuery] int id)
        {
            ...
            if (orderItems == null)
                return NotFound($"—остовл€ющие заказа с id заказа {id} не найден");

            return Ok(orderItems);
        }

        //—оздание состовл€ющей заказа
        ...
        public async Task<ActionResult<OrderItem>> CreateOrderItem([FromBody] CreateOrderItemRequest dto)
        {
            ...
            if (order == null)
                return BadRequest($"«аказ с id {dto.OrderId} не найден");

            var product = await _productRepository.GetProductsById(dto.ProductId);

            if (product == null)
                return BadRequest($"ѕродукт с id {dto.ProductId} не найден");
        ...
        public async Task<IActionResult> UpdateOrderItem([FromQuery] int id, [FromBody] UpdateOrderItemResponce dto)
        {
            ...
            if (id != dto.Id)
                return BadRequest("Ќесоответсвие id между URL-адресом и основным текстом");
